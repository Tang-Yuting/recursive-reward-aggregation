...
from recursive_stable_baselines3.recursive_common.buffers import RolloutBuffer
from recursive_stable_baselines3.recursive_common.on_policy_algorithm import OnPolicyAlgorithm, OnPolicyAlgorithm_multi_output
...
class Recursive_PPO(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # Value loss using the TD(gae_lambda) target, rollout_data.returns: target.
    ...
    def learn(
        self: SelfRecursive_PPO,
        total_timesteps: int,
        callback: MaybeCallback = None,
        log_interval: int = 1,
        tb_log_name: str = "Recursive_PPO",
...
class Recursive_PPO_multi_output(OnPolicyAlgorithm_multi_output):
    ...
    def learn(
        self: SelfRecursive_PPO_multi_output,
        total_timesteps: int,
        callback: MaybeCallback = None,
        log_interval: int = 1,
        tb_log_name: str = "Recursive_PPO",
        reset_num_timesteps: bool = True,
        progress_bar: bool = False,
    ) -> SelfRecursive_PPO_multi_output:
        return super().learn(
            total_timesteps=total_timesteps,
            callback=callback,
            log_interval=log_interval,
