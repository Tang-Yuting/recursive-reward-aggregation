...
from recursive_stable_baselines3.recursive_common.utils import obs_as_tensor, safe_mean
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions, taus, log_probs = self.policy(obs_tensor)
...
class OnPolicyAlgorithm_multi_output(BaseAlgorithm):
    ...
    def __init__(
        ...
        use_sde: bool,
        sde_sample_freq: int,
        rollout_buffer_class: Optional[type[RolloutBuffer_multi_output]] = None,
        rollout_buffer_kwargs: Optional[dict[str, Any]] = None,
        stats_window_size: int = 100,
        tensorboard_log: Optional[str] = None,
        monitor_wrapper: bool = True,
    ...
    def _setup_model(self) -> None:
        ...
        self.policy = self.policy_class(  # type: ignore[assignment]
            self.observation_space, self.action_space, self.lr_schedule, output_feature_num=self.output_feature_num, use_sde=self.use_sde, **self.policy_kwargs
        )
        self.policy = self.policy.to(self.device)
        # Warn when not using CPU with MlpPolicy
        self._maybe_recommend_cpu()
    ...
    def learn(
        self: SelfOnPolicyAlgorithm_multi_output,
        total_timesteps: int,
        callback: MaybeCallback = None,
        log_interval: int = 1,
        tb_log_name: str = "OnPolicyAlgorithm_multi_output",
        reset_num_timesteps: bool = True,
        progress_bar: bool = False,
    ) -> SelfOnPolicyAlgorithm_multi_output:
        iteration = 0
