...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        # self.value_net_for_mean = nn.Linear(self.mlp_extractor.latent_dim_vf, 2)
    ...
    def predict_taus(self, obs: PyTorchObs) -> th.Tensor:
        features = super().extract_features(obs, self.vf_features_extractor)
        latent_vf = self.mlp_extractor.forward_critic(features)
        return self.value_net(latent_vf)


class ActorCriticPolicy_multi_output(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        # self.value_net = nn.Linear(self.mlp_extractor.latent_dim_vf, 1)
        self.value_net_multi_output = nn.Linear(self.mlp_extractor.latent_dim_vf, self.output_feature_num)
        # Init weights: use orthogonal initialization
        # with small initial weight for the output
        if self.ortho_init:
            # TODO: check for features_extractor
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        if self.share_features_extractor:
            ...
            latent_vf = self.mlp_extractor.forward_critic(vf_features)
        # Evaluate the values for the given observations
        values = self.value_net_multi_output(latent_vf)
        distribution = self._get_action_dist_from_latent(latent_pi)
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        actions = actions.reshape((-1, *self.action_space.shape))  # type: ignore[misc]
    ...
    def evaluate_actions_multi_output(self, obs: PyTorchObs, actions: th.Tensor) -> tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        Evaluate actions according to the current policy,
        given the observations.
        ...
        # Preprocess the observation if needed
        features = self.extract_features(obs)
        # print("self.share_features_extractor", self.share_features_extractor)
        if self.share_features_extractor:
            latent_pi, latent_vf = self.mlp_extractor(features)
        else:
            pi_features, vf_features = features
        ...
        #     print("Warning: `features` contains NaN values!", features)
    ...
    def predict_taus_multi_output(self, obs: PyTorchObs) -> th.Tensor:
        features = super().extract_features(obs, self.vf_features_extractor)
        latent_vf = self.mlp_extractor.forward_critic(features)
        return self.value_net_multi_output(latent_vf)
