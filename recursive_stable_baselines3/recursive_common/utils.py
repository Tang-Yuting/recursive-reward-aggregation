...
import recursive_stable_baselines3 as sb3
...
def explained_variance_multi_output(y_pred: np.ndarray, y_true: np.ndarray, recursive_type) -> float:
    if recursive_type == "mean_multi_output":
        y_pred_sum, y_true_length = y_pred[...,0], y_true[...,1]
        y_pred_ = y_pred_sum / y_true_length
        var_y = np.var(y_true)
        return np.nan if var_y == 0 else float(1 - np.var(y_true - y_pred_) / var_y)
    elif recursive_type == "range_multi_output":
        y_pred_max, y_true_min = y_pred[..., 0], y_true[..., 1]
        y_pred_ = y_pred_max - y_true_min
        var_y = np.var(y_true)
        return np.nan if var_y == 0 else float(1 - np.var(y_true - y_pred_) / var_y)
    elif recursive_type == "p-mean_multi_output": # p = 2
        y_pred_p_mean, y_true_length = y_pred[..., 0], y_true[..., 1]
        y_pred_ = y_pred_p_mean
        var_y = np.var(y_true)
        return np.nan if var_y == 0 else float(1 - np.var(y_true - y_pred_) / var_y)
    elif recursive_type == "sum_variance_multi_output":
        y_pred_sum, y_pred_sum_square, y_pred_length = y_pred[..., 0], y_pred[..., 1], y_pred[..., 2]
        y_pred_variance = (y_pred_sum_square/y_pred_length) - (y_pred_sum/y_pred_length)**2
        y_pred_ = y_pred_sum - 0 * y_pred_variance
        var_y = np.var(y_true)
        return np.nan if var_y == 0 else float(1 - np.var(y_true - y_pred_) / var_y)
    elif recursive_type == "sum_range_multi_output":
        y_pred_sum, y_pred_max, y_pred_min = y_pred[..., 0], y_pred[..., 1], y_pred[..., 2]
        y_pred_range = y_pred_max - y_pred_min
        y_pred_ = y_pred_sum - 0 * y_pred_range
        var_y = np.var(y_true)
        return np.nan if var_y == 0 else float(1 - np.var(y_true - y_pred_) / var_y)
